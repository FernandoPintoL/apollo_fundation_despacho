...
import {
  authenticateRequest,
  getCacheStats,
} from './middleware/authentication-unified.js';
import type { AuthenticatedRequest } from './middleware/authentication-unified.js';
    ...
    return new RemoteGraphQLDataSource({
      ...
      willSendRequest({ request }) {
        ...
        request.http?.headers.set(
          'X-API-Key',
          process.env.API_KEY_ADMIN || 'admin-key-change-in-production-12345'
        );
      },
    });
  },
 ...
 */
app.get('/sandbox', (_req: Request, res: Response) => {
...
startupPromise.then(() => {
  ...
  if (process.env.NODE_ENV !== 'production') {
    app.get('/schema', async (_req: Request, res: Response) => {
      const schema = (apolloServer as any).schema;
      if (schema) {
        try {
          ...
          logger.error('Error printing schema', error as Error);
     ...
     */
    app.get('/auth/cache-stats', (_req: Request, res: Response) => {
...
╚════════════════════════════════════════╝
  ...
  logger.error('Error during Apollo Gateway startup', err as Error);

  // In resilient mode, we log the error but may continue
  // Service health will be monitored via /health/detailed endpoint
  logger.warn(
    'Gateway is starting in resilient mode. Services will be discovered as they become available.'
  );
